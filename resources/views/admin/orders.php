	...
	if ($action == "details"){
		...
		<div class="title">
			...
			if(count($products)>0){
				...
				$ids = rtrim($ids, ',');
				$order_products = \App\Product::whereRaw("id IN (".$ids.")")->orderby('id','desc')->get();
				...
				if (!empty($order->coupon)) {
					...
					if (\App\Coupon::where("code",$order->coupon)->count() > 0){
						$coupon_data = \App\Coupon::where("code",$order->coupon)->first();
						echo '<div class="item order text-right"> Coupon : <b>'.$coupon_data->discount.$coupon_data->type.'</b></div>';
					}
				}
				if ($order->shipping != 0) {
					// Check if coupon is valid
					echo '<div class="item order text-right"> Shipping : <b>'.currency($order->shipping).'</b></div>';
				}
				echo '<div class="item order text-right">Total : <b>'.currency($order->summ).'</b></div>';
			}
		echo '</div>
		...
		foreach ($orders as $order){
			...
			if(count($products)>0){
				...
				echo '<b>Total : '.currency($order->summ).'</b>';
			}
