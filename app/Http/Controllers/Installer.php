...
class Installer extends Controller
{
	...
	public function database()
	{
		...
		if (isset(request()->install)){
			$conn = @mysqli_connect(request()->host, request()->user, request()->password);
			if ($conn) {
				if (@mysqli_select_db($conn, request()->name)){
					  $config_code = file_get_contents('.env');
					  $config_code = str_replace('{host}', request()->host,  $config_code);
					  $config_code = str_replace('{database}', request()->name, $config_code);
					  $config_code = str_replace('{username}', request()->user, $config_code);
					  $config_code = str_replace('{password}', request()->password, $config_code);
					if(!is_writable(".env"))
					{
	...
	public function configurations()
	{
		...
		if (isset(request()->install)){
			...
			DB::statement(DB::raw('CREATE TABLE `slides` (
			  `id` int(11) NOT NULL,
			...
			DB::statement(DB::raw('CREATE TABLE `countries` (
			  `id` int(11) NOT NULL,
			  `iso` char(2) NOT NULL,
			...
			DB::statement(DB::raw('CREATE TABLE `languages` (
			  `id` int(11) NOT NULL,
			...
			(1, \'English\', \'en\'),
			(2, \'French\', \'fr\');'));
			DB::statement(DB::raw('CREATE TABLE `password_resets` (
			...
			DB::statement(DB::raw('CREATE TABLE `currencies` (
			  `id` int(11) NOT NULL,
			  `name` text COLLATE utf8_unicode_ci NOT NULL,
			  `code` text COLLATE utf8_unicode_ci NOT NULL,
			...
			DB::statement(DB::raw('CREATE TABLE `referrers` (
			...
			DB::statement(DB::raw('CREATE TABLE `translations` (
			  `id` int(11) NOT NULL,
			  `lang` mediumtext COLLATE utf8_unicode_ci NOT NULL,
			  `word` text COLLATE utf8_unicode_ci NOT NULL,
			  `translation` text COLLATE utf8_unicode_ci NOT NULL
			) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;'));
			DB::statement(DB::raw('INSERT INTO `translations` (`id`, `lang`, `word`, `translation`) VALUES
			(1, \'fr\', \'Shop\', \'Magazine\'),
			...
			(98, \'fr\', \'Sorry , This page does not exist\', \'Désolé , cette page n\'\'exist pas\'),
			(99, \'fr\', \'All fields are required !\', \'All fields are required !\'),
			(101, \'fr\', \'Language\', \'Langue\'),
			(183, \'fr\', \'Dress\', \'Dress\');'));
			DB::statement(DB::raw('CREATE TABLE `administrators` (
			  `id` int(11) NOT NULL,
			...
			DB::statement(DB::raw('ALTER TABLE `blog`
			  ADD PRIMARY KEY (`id`);'));
			DB::statement(DB::raw('ALTER TABLE `slides`
			  ADD PRIMARY KEY (`id`);'));
			DB::statement(DB::raw('ALTER TABLE `categories`
			  ADD PRIMARY KEY (`id`);'));
			DB::statement(DB::raw('ALTER TABLE `config`
			  ADD PRIMARY KEY (`id`);'));
			DB::statement(DB::raw('ALTER TABLE `countries`
			  ADD PRIMARY KEY (`id`);'));
			...
			DB::statement(DB::raw('ALTER TABLE `footer`
			  ADD PRIMARY KEY (`id`);'));
			DB::statement(DB::raw('ALTER TABLE `languages`
			  ADD PRIMARY KEY (`id`);'));
			...
			DB::statement(DB::raw('ALTER TABLE `payments`
			  ADD PRIMARY KEY (`id`);'));
			DB::statement(DB::raw('ALTER TABLE `currencies`
			  ADD PRIMARY KEY (`id`);'));
			...
			DB::statement(DB::raw('ALTER TABLE `tracking`
			  ADD PRIMARY KEY (`id`);'));
			DB::statement(DB::raw('ALTER TABLE `translations`
			  ADD PRIMARY KEY (`id`);'));
			DB::statement(DB::raw('ALTER TABLE `administrators`
			  ADD PRIMARY KEY (`id`);'));
			DB::statement(DB::raw('ALTER TABLE `blocs`
